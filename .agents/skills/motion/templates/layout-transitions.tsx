    ...
    <div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {cards.map((card) => (
            <motion.div
              ...
              className={`${card.color} text-white p-6    rounded-sm  cursor-pointer h-32 flex items-center justify-center`}
            >
      ...
      {selectedCard && (
        <motion.div
          ...
          className={`${selectedCard.color} text-white p-12    rounded-sm  cursor-pointer`}
        >
      ...
      <div className="space-y-2">
        {items.map((item) => (
          <motion.div
            ...
            className="p-4 bg-white border    rounded-sm  shadow-sm"
...
export function ScrollableList() {
  ...
  return (
    <motion.div
      ...
      className="h-96 overflow-auto border border-gray-200    rounded-sm  p-4 space-y-2"
    >
      {items.map((item) => (
        <motion.div
          key={item.id}
          ...
          className="p-4 bg-white border    rounded-sm  shadow-sm flex justify-between items-center"
        >
          <span>{item.text}</span>
          <button
      ...
      >
        {items.map((item) => (
          <motion.div
            key={item.id}
            layout
            className="p-6 bg-white border    rounded-sm  shadow-sm"
            transition={{ type: "spring", stiffness: 300, damping: 30 }}
          >
    ...
    <div className="space-y-2">
      {items.map((item, index) => (
        <motion.div
          ...
          className="p-4 bg-white border    rounded-sm  shadow-sm cursor-grab active:cursor-grabbing"
          transition={{ type: "spring", stiffness: 300, damping: 30 }}
        >
      ...
      <div className="columns-3 gap-4">
        {items.map((item) => (
          <motion.div
            key={item.id}
            layout
            className="mb-4 bg-gradient-to-br from-purple-500 to-pink-500    rounded-sm  p-4 text-white font-bold break-inside-avoid"
